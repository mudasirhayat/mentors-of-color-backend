...
const Breadcrumbs = ({
        ...
        >
          <Grid item>
            <MuiBreadcrumbs aria-label="breadcrumb" maxItems={maxItems || 8} separator={separatorIcon}>
              <Typography component={Link} to="/users" color="textSecondary" variant="h6" sx={{ textDecoration: 'none' }}>
  ...
  // items
  if ((item && item.type === 'item' && item?.role?.includes(user?.user_type)) || (item?.type === 'group' && item?.url) || custom) {
